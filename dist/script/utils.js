 ...
 * @method offset封装
 * @param {Object,Bollean} options {dom,bool)
 */
...
/**ajax封装------获取XML/JSON/PHP 数据
 * @method promise封装
 * @param {Object} options {url: '',type:'',data: {},dataType:'',success:()=>{}},error:()=>{})
 */
...
/**基于promise封装ajax------异步转换成同步
 * @method promise封装
 * @param {Object} options {url: '',type:'',data: {},dataType:''}.then(()=>{})
 */
...
/**用于jsonp调用----借用script链接引入页面
 * @method jsonp封装
 * @param {Object} options {url: '',data: '',jsonp: '',jsonpCallback: '',success: ()=>{}}
 */
...
/**设置cookie数据
 * @method setCookie()封装
 * @param {Object} options {key: '',val: '',days: '',domain: ''}
 */
function setCookie(options){
    if(!options.key||!options.val){
        throw new Error("设置失败，缺少重要参数");
    }
